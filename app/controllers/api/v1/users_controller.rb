class Api::V1::UsersController < ApplicationController
    ...
    def index
        json_response({ status: 'success', message: 'users loaded', data: UserSerializer.new(User.excluding(User.find_by role: 'Admin')).serializable_hash })
    end
    ...
    def create
        ...
        if user.save
           json_response({ status: 'success', message: 'User successfully added', data: UserSerializer.new(user).serializable_hash }, :created)
        else
    ...
    def set_admin
        if User.exists?
           json_response({status: 'error', message: 'You dont have required privilege to complete this action' })
            ...
            if user.save
               json_response({ status: 'success', message: 'Account successfully created', data: UserSerializer.new(user).serializable_hash }, :created)
            else
